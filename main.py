...
def algoritmo1(A,B):
	ab = gera_ab(A,B)
	razao1 = vec[0][0]#na pos 0, pegue o indice 0 da tuple, que é a razão 
		...
		if(ab[k][0] > R_star): # se a razao do par iterado atualmente for maior que o R* atual
			...
			if (R_star < Rs): # Se a razao otima R* for menor que a razao atual R(S)
                ...
                while (len(S_star)>0): #Retira todos que estavam no subconjunto otimo S* e coloque na lista. O(n^2)
                	s = S_star[-1]#pega o ultimo elemento
                    ind_list.append(s)
...
def quickSort(arr,low,high,ao,bo,A,B): 
    if low < high: 
        pivot = custom_pivot(ao,bo,A,B,arr[low:high])
        # pi is partitioning index, arr[p] is now 
        # at right place 
        pi = partition(pivot,arr,low,high) 
  
        # Separately sort elements before 
        # partition and after partition 
        quickSort(arr, low, pi-1,ao,bo,A,B) 
        quickSort(arr, pi+1, high,ao,bo,A,B) 
        return pi
...
def algoritmo3(A,B):
	...
	#pivot = rec_sol(J,0,len(J),ao,bo,A,B)#J,ini,end,ab,A,B,ao,bo
	pivot = quickSort(J,0,len(J)-1,ao,bo,A,B)
	print('result: ',J)
	print('pivot: ',pivot)
	"""
	sum_a = ao
