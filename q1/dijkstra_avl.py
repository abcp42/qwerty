...
def dijkstra(vertice_list,src,num_ver):
  ...
  from avl import AVL
  from avl2 import Tree
  from avl3 import AVLTree
  #tree = AVL()
  #tree.insert(vertice_list[src].dist,src)#avl works with indexes or keys only
  tree = Tree()
  
  #tree = AVLTree()
  #print('size:',tree.get_element_count())
    
  tree.insert(vertice_list[src].dist,src)
  ...
  while(tree.size>0):
    ...
    #v_min_node = tree.find_min()# 1.1
    v_min_node = tree.get_min()
    #print('min node:',v_min_node.key)
    v_min_index = v_min_node.key
    if(v_min_index==None):
      v_min_index = 0
    ...
    a = tree.delete(v_min_node.value)
    #print('deleted: ',a.key)
    ...
    for key in v_min.adjs:# 1.3
      ...
      if(adj_ver.visited==0 and  v_min.dist + v_min.weights[key] < adj_ver.dist ):
        ...
        #print('inserted: ',adj_ver.id)
        tree.insert(adj_ver.dist,adj_ver.id)
