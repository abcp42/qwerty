...
def search2(goal_level,begin_step,end_step,accumulated_positions):
	...
	if(testJar(step,goal_level)):
		#normalize interval
		return lineSearch(1,end_step,goal_level,accumulated_positions)#busca linear a partir do intervalo anterior

	begin_step+=1
	return search2(goal_level,begin_step,end_step,accumulated_positions)

def search22(goal_level,begin_step,end_step,accumulated_positions):
	last_step = 0
	while(1):
		#step_size <- n^((k - 1)/k)
		step = begin_step**2
...
def search33(goal_level,begin_step,end_step,accumulated_positions):
	last_step = 0
	#step_size <- n^((k - 1)/k)
	while(1):
		step = begin_step**3
		if(testJar(step,goal_level)):
			...
			accumulated_positions+= (begin_step-1)**3 #go saving to remember the true floor/ goal
			print("new interval: from 1 to ",end_step)
			return search22(goal_level,1,end_step,accumulated_positions)
...
def search4(goal_level,begin_step,end_step,accumulated_positions):
	...
	begin_step+=1
	return search4(goal_level,begin_step,end_step,accumulated_positions)

def search44(goal_level,begin_step,end_step,accumulated_positions):
	last_step = 0
	#step_size <- n^((k - 1)/k)
	while(1):
		step = begin_step**4
		if(testJar(step,goal_level)):
			...
			accumulated_positions+= (begin_step-1)**4 #go saving to remember the true floor/ goal
			print("new interval: from 1 to ",end_step)
			return search33(goal_level,1,end_step,accumulated_positions)
...
search44(tests[0] ,1,maxval,0)
#searchk(4,33445500,1,50000000,0)
timer.stop()
